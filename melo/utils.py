...
def load_checkpoint(checkpoint_path, model, optimizer=None, skip_optimizer=False):
    ...
    checkpoint_dict = torch.load(checkpoint_path, map_location="cpu", weights_only=False) # [수정됨] PyTorch 보안 경고 방지를 위해 weights_only=False 추가
...
def latest_checkpoint_path(dir_path, regex="G_*.pth"):
    ...
    if not f_list:  # 리스트가 비어있으면 None 반환
        return None
...
def get_hparams(init=True):
    ...
    if init:
        ...
        # If not init, read the saved config
        if os.path.exists(config_save_path):
            data = read_file_with_fallback_encoding(config_save_path)
        else:
            # Fallback to original config if saved one doesn't exist
            data = read_file_with_fallback_encoding(config_path)
...
def clean_checkpoints(path_to_models="logs/44k/", n_ckpts_to_keep=2, sort_by_time=True):
    ...
    def name_key(_f):
        # Gracefully handle filenames that don't match the expected pattern
        match = re.search(r"._(\d+)\.pth", _f)
    ...
    to_del_g = x_sorted("G")
    to_del_d = x_sorted("D")
    to_del_dur = x_sorted("DUR")
    
    to_del = (
        to_del_g[:-n_ckpts_to_keep]
        + to_del_d[:-n_ckpts_to_keep]
        + to_del_dur[:-n_ckpts_to_keep]
    )
    ...
    def del_routine(x):
        full_path = os.path.join(path_to_models, x)
